...
##-----------------------------------------------------------------------------
## Função iterativa/interativa
anima <- function(panel){
    layout(matrix(c(1,1,1,2,3,4), ncol=3, by=TRUE))
    for (i in 1:length(u)) {
        ## Velocidade da animação
        Sys.sleep( time = panel$time)
        ## ---- Gráfico 1
...
panel <- rp.control("Tranformação Intergral de Probabilidade", time = 0.40)
rp.button(panel = panel, action = anima, title = "Executar animação")


##=============================================================================
## Melhorias: utlizar as funções rp.tkrplot e rp.tkrreplot para o
## gráfico ser exibido na mesma janela do botão. Incluir controladores de
## play/pause (button) e velocidade da animação(doublebutton).
